Param(
		[Parameter(Mandatory=$true)][String]$enviromentUrl,
        [Parameter(Mandatory=$true)][String]$username
    )

$Invocation = (Get-Variable MyInvocation).Value
$DirectoryPath = (Get-Item $Invocation.MyCommand.Path).Directory.FullName
$LogPath = $DirectoryPath + "\GetAllTurnOffFlow_$([DateTime]::Now.ToString("yyyyMMddHHmmssfff")).log"
$CsvPath = $DirectoryPath + "\GetAllTurnOffFlow_$([DateTime]::Now.ToString("yyyyMMddHHmmssfff")).csv"
...
Function CheckPowershellEnvironment{

    $HasCrmConnector = Get-Module -ListAvailable -FullyQualifiedName "Microsoft.Xrm.Tooling.CrmConnector.PowerShell"
    if($HasCrmConnector.Count -eq 0)
    {
        try
        {
            Install-Module Microsoft.Xrm.Tooling.CrmConnector.PowerShell -AllowClobber -Force -Scope CurrentUser
            Write-LogInfo -LogPath $LogPath -Message "Don't have Microsoft.Xrm.Tooling.CrmConnector.PowerShell, install successful." -ToScreen
        }
        catch
        {
            Write-LogError -LogPath $LogPath -Message "Failed to install Microsoft.Xrm.Tooling.CrmConnector.PowerShell, Message: [$($_.ToString())]" -ToScreen -Exception $_
        }
    }
    $HasXrmData = Get-Module -ListAvailable -FullyQualifiedName "Microsoft.Xrm.Data.PowerShell"
    if($HasXrmData.Count -eq 0)
    {
        try
        {
            Install-Module Microsoft.Xrm.Data.PowerShell -AllowClobber -Force -Scope CurrentUser
            Write-LogInfo -LogPath $LogPath -Message "Don't have Microsoft.Xrm.Data.PowerShell, install successful." -ToScreen
        }
        catch
        {
            Write-LogError -LogPath $LogPath -Message "Failed to install Microsoft.Xrm.Data.PowerShell, Message: [$($_.ToString())]" -ToScreen -Exception $_
        }
    }
}
...
Function GetFlow{
    try
    {
        $connection = Connect-CRMOnline -Username $username -ServerUrl $enviromentUrl
        

        if($connection.IsReady -ne $True)
        {
            $errorDescr = $connection.LastCrmError
            Write-LogInfo -LogPath $LogPath -Message "Connection not established: $errorDescr" -ToScreen
        }
        else
        {
            Write-LogInfo -LogPath $LogPath -Message "Connection to $($connection.ConnectedOrgFriendlyName) successful" -ToScreen
            Write-LogInfo -LogPath $LogPath -Message "Get Flows that are turned off" -ToScreen
...
"@;
            try
            {
                $flows = (Get-CrmRecordsByFetch  -conn $connection -Fetch $fetchFlows -Verbose).CrmRecords
                Write-LogInfo -LogPath $LogPath -Message "Get Flows successful, flow count: $($flows.Count)" -ToScreen
                    ...
                    <fetch>
                      <entity name='solution' >
                        ...
                        <link-entity name='solutioncomponent' from='solutionid' to='solutionid' >
                          <filter type='and'>
                            <condition attribute='objectid' operator='eq' value='$workflowid' />
                          </filter>
                        </link-entity>
                      </entity>
                    </fetch>
"@;
                    ...
                    try
                    {
                        $solutionInfo = (Get-CrmRecordsByFetch  -conn $connection -Fetch $fetchSolution -Verbose).CrmRecords
                        Write-LogInfo -LogPath $LogPath -Message "Get flow solution info successful, flow uniqueid:[$($flow.workflowidunique)], flow name: [$($flow.name)]" -ToScreen
                    }
                    catch
                    {
                        Write-LogError -LogPath $LogPath -Message "Get flow solution info failed, flow uniqueid:[$($flow.workflowidunique)], Message: [$($_.ToString())]" -ToScreen -Exception $_
                    }
                    $flowSuccessful = New-Object PSObject
                    $flowSuccessful | Add-Member -MemberType NoteProperty -name "workflowidunique" -value $flow.workflowidunique
                    $flowSuccessful | Add-Member -MemberType NoteProperty -name "workflowname" -value $flow.name
                    $solutionname = ''
                    if($solutionInfo.Count -gt 1)
                    {
                        Foreach($info in $solutionInfo)
                        {
                            $solutionname += $info.uniquename+','
                        }
                        $solutionname.TrimEnd(',')
                    }
                    else
                    {
                        $solutionname = $solutionInfo.uniquename
                    }
                    $flowSuccessful | Add-Member -MemberType NoteProperty -name "solutionname" -value $solutionname
                    $results += $flowSuccessful
                }
                $results | Export-Csv -Path $CsvPath -NoTypeInformation -Encoding UTF8 -Append
            }
            catch
            {
                Write-LogError -LogPath $LogPath -Message "Get flows failed, Message: [$($_.ToString())]" -ToScreen -Exception $_
            }
        }
    }
    catch
    {
        ...
        Write-LogError -LogPath $LogPath -Message ", Message: [$($_.ToString())]" -ToScreen -Exception $_
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB1VcY0TkZAWQCh
# pio+gXDIg9SmcHRkcO82LkXlCN1y0aCCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCCEKaOwJlA+7A5+gap+RkSBmHw+eN2fdjD4LfjWrtprPzANBgkq
# hkiG9w0BAQEFAASCAYCrqfv13cZNcNWAcenGO6JqBoSR7uWozjssDVtvzkc3PGuk
# qtITz4joNSnO+ft3zCmtWBbq3vfRbnygHTq33dLxfYU6Hp5x3mLhGvNNnnATMy4n
# 004USW/HWbU+4g0xbrYKi/2KY+5htOTivfdEpvrIPOpijY7fnLMgmgvMpREqj2s8
# qkE4k2ZClizoH/9sBJVVo2C2tBbcqapzB/gjTnWgazRgOMbqT4bad8Cu8zO9nPdW
# 612ENy26gZfP4iDhbicb+02fw87PFxGjZXStbOfbXwHdyTfSYbOAksLTr9uLgSJh
# /J+o3CQlREfmDPSTRSe7wNtZfVpLZRS2z9DJQjGJUjcY2nfQ88hCZdOpkIQLvAT6
