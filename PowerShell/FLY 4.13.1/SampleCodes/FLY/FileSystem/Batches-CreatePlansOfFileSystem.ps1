...
# For example: 
# $ImportPlanTemplate = 'ImportPlanTemplate-OD.csv'/'ImportPlanTemplate-SP.csv'
# Specify the name of the imported template, Please refer to the PlanCSVs folder in the current folder for specific template content
$ImportPlanTemplate = 'ImportPlanTemplate-SP.csv'

try{
	$CSVFilePath = Join-Path -Path $Path -ChildPath 'PlanCSVs'

	$NewFilePath = $CSVFilePath+"\"+$DestinationType+"-ImportPlanTemplate-result.csv"
	$ImportCsv = Import-Csv -Path $CSVFilePath"\"$ImportPlanTemplate
	...
	try{
		...
		$sourceConnectionID = $connection."Source Account Name"
        $sourceCredential = New-CredentialObject -Username $connection."Source Credential Account" -Password $connection."Source Credential Password"

		
		$destAccount =  CheckData -data $connection."Destination Account Name"
		$destProfile =  CheckData -data $connection."Destination Profile Name"
		...
		foreach($csv in $Csvs)
		{
				...
				foreach($mapping in $mappings)
				{
					...
					$Source = New-FSPathObject -Level $mapping."Source Level" -Path $mapping."Migrate From"

					$Destination = New-FSMigrationSharePointObject -Level $mapping."Dest Level" -Url $mapping."Migrate To"

					$MappingContent = New-FSMappingContentObject -Source $Source -Destination $Destination -Method $mapping."Method"

					$mappingList += $MappingContent
				}
			   
				$Mappings = New-FSMappingObject -SourceConnectionId $sourceConnectionID -SourceCredential $sourceCredential -DestinationConnectionId $connection."Destination Connection" -Contents $mappingList -SourceAgents $SourceAgents -DestinationType $DestinationType
				...
				$PlanSettings = New-FSPlanSettingsObject -MigrationMode HighSpeed -DatabaseId $Database -PolicyId $Policy -DisplayName $PlanNameLabel -Schedule $Schedule -PlanGroups $PlanGroups -MetaData:$CustomizeMetadata -MetaFilePath $MetaDataFilePath
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCY/3lveXqC/zeg
# E4xkpkpRTfvwsta0VLpbZLSOhKt956CCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCCIc8BwdUwIz6yzgvSYYxfj/XOZlTNQuVF2mjufY2CxiDANBgkq
# hkiG9w0BAQEFAASCAYCKE3cqlrgL4HRoEDf3DYgUqGM3FHYnWTl1VRT7D18ez20e
# dDj7dFEhZ+OkQM0ZK42ornP/nRPDdm2yyYSignwMAWed0enj8CFpTNC6JHqxIt6F
# jf/9B/dQEzbuVkyxZUPaJ95411HAm0/l6L+M0Ua3e6wBb+gu4vJdNMencjfIoman
# QNltlPhNsbg9PX7WluBtjiAxEU98WbZo1pERadmnpME10k7jsfRPEJ/l6/RFEJth
# 5Bp6xqlabc77SutVTt2Kiutw6TA1ZZmmWLBKVxCBcr/ePVZNG+rWrotEX+gsysUA
