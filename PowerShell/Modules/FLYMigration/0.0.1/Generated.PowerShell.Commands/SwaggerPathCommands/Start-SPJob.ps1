...
#>
function Start-SPJob
{
    [OutputType([Microsoft.PowerShell.FLYMigration.v001.Models.ServiceResponsePlanSummaryModel])]
    [CmdletBinding(DefaultParameterSetName='SPJob_Start')]
    param(    
        [Parameter(Mandatory = $true, ParameterSetName = 'SPJob_Start')]
        [Microsoft.PowerShell.FLYMigration.v001.Models.SharePointJobExecutionModel]
        $JobSettings,
...
'@ 
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('SPJob_Start' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation StartWithHttpMessagesAsync on $APElementsFLYAPI.'
        $taskResult = $APElementsFLYAPI.SPJob.StartWithHttpMessagesAsync($JobSettings)
    } else {
