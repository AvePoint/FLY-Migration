...
#>
function Find-FSJobByPlan
{
    [OutputType([Microsoft.PowerShell.FLYMigration.v001.Models.ServiceResponsePageResultViewModelListJobSummaryModel])]
    [CmdletBinding(DefaultParameterSetName='FSJobByPlan_Find')]
    param(    
        [Parameter(Mandatory = $false, ParameterSetName = 'FSJobByPlan_Find')]
        ...
        [Parameter(Mandatory = $true, ParameterSetName = 'FSJobByPlan_Find')]
        [System.String]
        $Id,
...
'@ 
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('FSJobByPlan_Find' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation FindWithHttpMessagesAsync on $APElementsFLYAPI.'
        $taskResult = $APElementsFLYAPI.FSJobByPlan.FindWithHttpMessagesAsync($Id, $PageNumber, $PageSize)
    } else {
