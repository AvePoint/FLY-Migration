...
$ConversationASMessageObjct = New-ConversationsMigrationSettingsObject -Style ConversationsAfter -MigrateConversationsTimer "2020-07-04"
...
foreach($connection in $connctions)
{
    ...
    foreach($csv in $Csvs)
    {
        ...
        foreach($mapping in $mappings)
        {
            ...
            $Source = New-MSTeamsObject -Mailbox $mapping."Source Mailbox"
        ...
        $mappings = $MappingContent = New-MSTeamsMappingObject -SourceConnectionId $connection."Source Connection" -DestinationConnectionId $connection."Destination Connection" -Contents $mappingList
        ...
        $PlanSettings = New-MSTeamsPlanSettingsObject -NameLabel $PlanNameLabel -PolicyId $Policy -DatabaseId $DatabaseId -Schedule $Schedule -PlanGroups $PlanGoups -OnlyMigrateDocumentsLibrary -MigrateAsHTML -ConversationsMigrationAsHTML $ConversationsAsHTMLObject -MigrateAsMessage  -ConversationsMigrationAsMessages $ConversationASMessageObjct  -MigrateMembers -MigrateGroupPlanner 
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUjF3Kyd8jMbf3f2ksjXrRNb11
# qu+gghL+MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUSSEBIcwmae92p/sTgb2x
# lrZozLUwDQYJKoZIhvcNAQEBBQAEggEAhMZ7wmDx8k2kckHS8lug8iqtuGyI+I3i
# 8Hh5LtqLKhp0xmi9ME0hPhce5xYeHqALMCB6yyTmXO0WwkSDoYhzY69H4NobyCM8
# 46qIAV5yhnJOKRzc4EKpkgxXLz3UERhbyKVr78OljgpfLgbu/grEUjcWPIaeFmgN
# qhgFeuFtqW++rpNLVlCqdHZsLCwaLBmRgWC7w0PCgdBU1eNf9c7nrAt/Paecke08
# mtEFDlQSdMs1qF1RfvVIDhyrPrFCt9jpO5FOPC8/EI+ra7d6t6Cxc6oYVxfv8mNQ
# BYpWZA9CUKUDLQXOaRE5myE4LpCpLJoK3yCPuubRe44uXDBpBH9yx6GCAgswggIH
...
# Fw0yMDEyMDkwNjU0NDZaMCMGCSqGSIb3DQEJBDEWBBShwNJOtqoUQCyI3VJnY0jH
# V39aAjANBgkqhkiG9w0BAQEFAASCAQBCkyOhKmLRPqYHqjyH02NqKycTpADgKBkk
# BYj+t7IYrK4KnAzMydw1it1/zeegBoWU8Z9j0+Pcfvb2CmLL2ysoqNlYlNrDCZHl
# 7qUqCawzTIkc2juscSf6kII/8U0uLN8KHRsm4NTUg7A4m4zG2qNMCgaM33Kkhh3Z
# +t2oOTsJ5lGrDZFLM/wsfUYbG24V0FHLOcWVwEaR8xgStO95BsS0LNyeCUzBf7gT
# KVKNC/+R9V+WdccARJpDVtA3jaX72PJPIDkTZBEpJj3UFujgsrbZU9S2Z4Z50MoG
# 6oHhy7zeZirGBc3X0ZoymRACTMPNR4z6nOUDc6CFLfDWtJq6ePD3
