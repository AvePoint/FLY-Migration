...
try{
	...
	$connections  = Import-Csv -Path $NewFilePath
		...
		{
			try{
				$ConnCredential = New-BasicCredentialObject -Username $connection."Service Account" -Password $connection."Password"
				$SourceConnection = New-PSTFileConnectionOptionObject -BasicCredential $ConnCredential -Path $connection."UNC Path"
				$DestConnectionType = Set-connectionType -type $connection."Destination Type"
		        $DestinationConn = Set-connection -id $connection."Destination Connection" -isOnlie $DestConnectionType
			   ...
			   foreach($csv in $Csvs)
			   {
				...
				{
					$Source = New-PSTFileObject -Path $mapping."PST File Path" -Password $mapping."Password"
					$Destination = New-ExchangeMailboxObject -Mailbox $mapping."Destination" -MailboxType $mapping."Destination Type"
					
					$MappingContent = New-PSTFileMappingContentObject -Source $Source -Destination $Destination
					$mappingList += $MappingContent
				}
				$Mappings = New-PSTFileMappingObject -PstFileConnection $SourceConnection  -Destination $DestinationConn -Contents @($mappingList) -VerifyMapping:$VerifyMapping -SourceAgents $SourceAgents
				...
				$PlanSettings = New-PSTFilePlanSettingObject -DisplayName $PlanNameLabel -PolicyId $Policy -DatabaseId $Database  -PlanGroups $PlanGroups -Schedule $Schedule 
				$Plan = New-PSTFilePlanObject  $PlanSettings -Mappings $Mappings
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAVpRRWgHQ6gTml
# Q2FMzGDNP0UVb+JSP2C9mH2lVuo98KCCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCBBWxfB0+HN8U/Ud4lYfod2pRvNCWZ55liuFNlDAJIQ2zANBgkq
# hkiG9w0BAQEFAASCAYCZ1sZYEIyb1CrA51iRHaq1YOCCiegqjwIzbCookJ8DE9Ny
# dLrMGU5radorFirkkfVvvhzieQJ/2gEWleZ5ElY9lCP0p6/X7VHAZPHm/vkmtRS4
# A76bOGk20Qp410QbdwVJiI+muxhGpGAej+yyS1dWvrq8Xbty5WI88Hdt7jAoWW76
# p6qH0+OKgYdwddfs4kZFAmuczZJG5ttJ0X8YRFioh71Y/a3qBfJnPx1HsrGovKdl
# Z9q3W5LiBJ6pWlCBPvwu1CbgJmCxpVBAvWjrwZ0byYLaPxR4GExbEyxnhXSIeW8J
# pEa2g8jWlHqdTJhLBVwmrTp1T3IuvhuQAwo7obJSKAPmWe+KfrxXFT+n2UnfHuQX
# 0WjZAJ5Gx+caEcblMRREwAHw6xVNp2qV2jhB6OTvyEOMtaXHIc1exzxAq3fAuGEy
