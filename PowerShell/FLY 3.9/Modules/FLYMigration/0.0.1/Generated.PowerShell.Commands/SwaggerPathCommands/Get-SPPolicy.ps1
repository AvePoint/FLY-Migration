...
{
    [OutputType([Microsoft.PowerShell.FLYMigration.v001.Models.ServiceResponseListPolicySummaryModel])]
    [CmdletBinding(DefaultParameterSetName='SPPolicy_Get')]
    param(    
...
'@ 
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('SPPolicy_Get' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation GetWithHttpMessagesAsync on $APElementsFLYAPI.'
        $taskResult = $APElementsFLYAPI.SPPolicy.GetWithHttpMessagesAsync()
    } else {
