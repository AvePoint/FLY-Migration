...
#>
function Find-SPJobByPlan
{
    [OutputType([Microsoft.PowerShell.FLYMigration.v001.Models.ServiceResponsePageResultViewModelListJobSummaryModel])]
    [CmdletBinding(DefaultParameterSetName='SPJobByPlan_Find')]
    param(    
        [Parameter(Mandatory = $false, ParameterSetName = 'SPJobByPlan_Find')]
        ...
        [Parameter(Mandatory = $true, ParameterSetName = 'SPJobByPlan_Find')]
        [System.String]
        $Id,
...
'@ 
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('SPJobByPlan_Find' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation FindWithHttpMessagesAsync on $APElementsFLYAPI.'
        $taskResult = $APElementsFLYAPI.SPJobByPlan.FindWithHttpMessagesAsync($Id, $PageNumber, $PageSize)
    } else {
