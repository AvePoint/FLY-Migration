...
function Get-FSPolicy
{
    ...
    [CmdletBinding(DefaultParameterSetName='FSPolicy_Get')]
    param(    
...
'@ 
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('FSPolicy_Get' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation GetWithHttpMessagesAsync on $APElementsFLYAPI.'
        $taskResult = $APElementsFLYAPI.FSPolicy.GetWithHttpMessagesAsync()
    } else {
