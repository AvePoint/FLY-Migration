...
function Get-FSJob
{
    ...
    [CmdletBinding(DefaultParameterSetName='FSJob_Get')]
    param(    
        [Parameter(Mandatory = $false, ParameterSetName = 'FSJob_Get')]
...
'@ 
    ...
    $skippedCount = 0
    $returnedCount = 0
    if ('FSJob_Get' -eq $PsCmdlet.ParameterSetName) {
        Write-Verbose -Message 'Performing operation GetWithHttpMessagesAsync on $APElementsFLYAPI.'
        $taskResult = $APElementsFLYAPI.FSJob.GetWithHttpMessagesAsync($PageNumber, $PageSize)
    } else {
