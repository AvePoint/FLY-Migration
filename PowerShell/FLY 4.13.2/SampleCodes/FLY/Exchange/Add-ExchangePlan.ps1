...
try
{
    ...
    #   $SourceConnOption = New-ExchangeOnPremisesConnectionOptionObject -ExchangeServerOption $ExchangeServer  -BasicCredential $Credential
    #   $SourceConnection = New-ExchangeConnectionObject -OnPremisesConnectionOption $SourceConnOption -SourceAgents $SourceAgents
    # On-Premises connection:
    #   $SourceConnOption = New-ExchangeOnPremisesConnectionOptionObject -ConnectionId 'Exchange On-Premises connection name or id'
    #   $SourceConnection = New-ExchangeConnectionObject -OnPremisesConnectionOption $SourceConnOption -SourceAgents $SourceAgents
    $SourceConnOption = New-ExchangeOnPremisesConnectionOptionObject -ConnectionId ''
    $SourceConnection = New-ExchangeConnectionObject -OnPremisesConnectionOption $SourceConnOption -SourceAgents $SourceAgents
    ...
    #   $Credential = New-BasicCredentialObject -Username 'Exchange Online username' -Password 'Exchange Online password'
    #   $DestinationConnOption = New-ExchangeOnlineConnectionOptionObject -BasicCredential $Credential
    #   $DestinationConnection  = New-ExchangeConnectionObject -OnlineConnectionOption $DestinationConnOption -DestinationAgents $DestinationAgents
    # online connection:
    ...
    # For example:
    # User Mailbox: $SourceMapping1 = New-ExchangeMailboxObject -Mailbox 'user@contoso.onmicrosoft.com' -MailboxType UserMailbox
    # User Mailbox: $SourceMapping1 = New-ExchangeMailboxObject -Mailbox 'user@contoso.onmicrosoft.com\folder' -MailboxType UserMailbox
    # Archive Mailbox: $SourceMapping1 = New-ExchangeMailboxObject -Mailbox 'user@contoso.onmicorsoft.com(In-Place Archive Mailbox)' -MailboxType ArchiveMailbox
    # Shared Mailbox: $SourceMapping1 = New-ExchangeMailboxObject -Mailbox 'shared@contoso.onmicrosoft.com' -MailboxType SharedMailbox
    # Resource Mailbox: $SourceMapping1 = New-ExchangeMailboxObject -Mailbox 'resource@contoso.onmicrosoft.com' -MailboxType ResourceMailbox
    # Microsoft 365 Group Mailbox: $SourceMapping1 = New-ExchangeMailboxObject -Mailbox 'group@contoso.onmicrosoft.com' -MailboxType Microsoft365GroupMailbox
    # Scheduling Mailbox: $SourceMapping1 = New-ExchangeMailboxObject -Mailbox 'user@contoso.onmicrosoft.com' -MailboxType SchedulingMailbox
    $SourceMapping1 = New-ExchangeMailboxObject -Mailbox '' -MailboxType UserMailbox
	$SourceMapping2 = New-ExchangeMailboxObject -Mailbox '' -MailboxType SchedulingMailbox
    ...
    # Scheduling Mailbox: $Destinationmapping1 = New-ExchangeMailboxObject -Mailbox 'user@contoso.onmicrosoft.com' -MailboxType SchedulingMailbox
    ...
    $MappingContent1 = New-ExchangeMappingContentObject -Source $SourceMapping1 -Destination $Destinationmapping1 -ConvertToSharedMailbox -MigrateArchiveMailbox -MigrateDeletonsFolder
	$MappingContent2 = New-ExchangeMappingContentObject -Source $SourceMapping2 -Destination $Destinationmapping2 -ConvertToSharedMailbox -MigrateArchiveMailbox -MigrateDeletonsFolder

    # Verify the mailboxes exist and are available for migration.
    # For example: $VerifyMapping = $True/$False
    $VerifyMapping = $False;

    $Mappings = New-ExchangeMappingObject -Source $SourceConnection -Destination $DestinationConnection -Contents @($MappingContent1,$MappingContent2) -VerifyMapping:$VerifyMapping
    ...
    $PlanSettings = New-ExchangePlanSettingsObject -DisplayName $PlanName -PolicyId $Policy -DatabaseId $Database -Schedule $Schedule -PlanGroups $PlanGroups -MigrateMailboxRules -SynchronizeDeletion -MigrateMailboxPermissions -MigrateContacts -MigrateAutoCompleteList
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC