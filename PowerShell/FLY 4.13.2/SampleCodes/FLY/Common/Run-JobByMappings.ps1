...
try
{
    # For example: $MigrationType = 'Full/Incremental/FailedObjectsOnly/GroupsOnly/PermissionOnly/IncrementalBaseTime'
    $MigrationType = 'Incremental'
    
    #There are two ways to set mapping data:
    #1. Import the mapping data file generated by the Get-PlanMappings.ps1 script, example:
    #   $Mappings = Import-Csv -Path 'C:\Program Files\APElements\FLY\Manager\PowerShell\SampleCodes\FLY\Common\ExportCsv\PLAN20240327152201390915.csv'
    #2. Manually input data, example:
    #   $MappingContent1 = New-MappingDetailObject -MappingId 'mapping id1' -PlanId 'plan id1'   
	#   $MappingContent2 = New-MappingDetailObject -MappingId 'mapping id2' -PlanId 'plan id2'
    #   $Mappings = @($MappingContent1,$MappingContent2)
    $MappingContent1 = New-MappingDetailObject -MappingId 'mapping id1' -PlanId 'plan id1'
    $MappingContent2 = New-MappingDetailObject -MappingId 'mapping id2' -PlanId 'plan id2'
    $Mappings = @($MappingContent1,$MappingContent2)

    # For more information about the -StartTime format, please see the  New-ScheduleObject.ps1 file in '...\SampleCodes\FLY\Common\' directory 
    # For example: $Settings = New-MappingJobExecutionObject -MigrationType Full/Incremental/FailedObjectsOnly/GroupsOnly/PermissionOnly/IncrementalBaseTime -StartTime (([Datetime]::Now).AddMinutes(2).ToString("o"))
    # Time-based Migration example: $Settings = New-MappingJobExecutionObject -MigrationType IncrementalBaseTime -BaseTime '2020-11-13 12:01:01+08:00'
    $Settings = New-MappingJobExecutionObject -MigrationType $MigrationType -Mappings $Mappings
    $Response = Start-MappingJob -BaseUri $BaseUri -APIKey $ApiKey -Settings $Settings
	$Response.Content|fl
}
Catch
{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCArWZXsZ+j1p0wj
# 7KXH2/hHSAg49nEk/pOkSAuyN4qF/6CCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCAv6J/mDeZKXJJg+DVCPFSZuYo1UbTaiaVBXqgu6xAffTANBgkq
# hkiG9w0BAQEFAASCAYB6mJH9j8ar2jRL5F10w5oemM/4mHYT589zTwt9L97cZJnT
# ijXdCjgSwF9OYlSM0BMUCIVeAhkoyw4u916CxSdxwwq9oR4NH57X6lx9S0SGxK6m
# i6dEm+rvJbkJObfp3rP341Pv6he5v6nr+kV/L4p0WtI8MvMDLdegLreS+CkFitD2
# phJvfukiLRSfcmjG2WmxMUjkWRPFB2AErRaK+o5cqyA69en0T91UW/IFX1Wh2Zgt
