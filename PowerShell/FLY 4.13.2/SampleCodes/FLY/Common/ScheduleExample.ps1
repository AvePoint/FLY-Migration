...
$schedule = New-ScheduleObject -StartTime '<StartTime>' -IntervalType Once
...
#If you wish to set the time in a format like the screenshot,you need to know what your time zone, e.g. you in East 8, you need to add 8.If you #are in West 5, you need to subtract 5.
#The example below will convert to run this plan at 8.50am BeiJing time  and run the incremental migration the next day.
$Schedule = New-SimpleScheduleObject -IntervalType Daily -StartTime 2021-05-03 09:50:00+08:00 -LastIncrementalMigrationStartTime "2021-05-06 09:50:00+08:00" 

#If you don't know your time zone, you can follow the example below to set it.
#If you want the schedule to be executed in 5 minutes, you need to fill in the AddMinutes() parameter with 5.

$Schedule = New-SimpleScheduleObject -IntervalType Daily -StartTime ([Datetime]::Now).AddMinutes(5).ToString('o') -LastIncrementalMigrationStartTime ([Datetime]::Now).AddDays(1).ToString('o') 

#([Datetime]::Now).AddMinutes(5).AddHours(2).ToString('o')  Current time plus two hours
#([Datetime]::Now).AddMinutes(5).AddMonths(2).ToString('o') Current time plus two Months

# plan group 
# Daily
$Schedule = New-ScheduleObject -IntervalType Daily -StartTime ([Datetime]::Now).AddMinutes(2).ToString('o')
# Hourly/Daily/Weeekly example
# No End Date
$Schedule = New-ScheduleObject -IntervalType Hourly -StartTime ([Datetime]::Now).AddMinutes(2).ToString('o') -Interval 10 -EndType NoEnd 
# End after occurrence(s)
$Schedule = New-ScheduleObject -IntervalType Hourly -StartTime ([Datetime]::Now).AddMinutes(2).ToString('o') -Interval 10 -EndType Occurrences -OccurrencesValue 10
# End by
$Schedule = New-ScheduleObject -IntervalType Hourly -StartTime ([Datetime]::Now).AddMinutes(2).ToString('o') -Interval 10 -EndType Time -EndTime ([Datetime]::Now).AddMinutes(5).ToString('o') 

# SIG # Begin signature block
# MIIoJwYJKoZIhvcNAQcCoIIoGDCCKBQCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB92FaIk79gPxgW
# B636fq1ZkCM9mifjiDSgucdBsCOZqqCCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCDn1yvCXMGcS5W/uYFCsvHFr0x6AwOtfP7WihFzKRYEMTANBgkq
# hkiG9w0BAQEFAASCAYDKUwY+xWVpo9TAPlHND59+w94ng6NswKMDsy7J9YBnKi0g
# v1n0GwRzpHGNvTMx3d/FgV9WIGxsMcG9N4xEhqHX4xzgBhJZibZJPXxrMcOlvArD
# Apv0QG6XFzGgEyikxrujR7AjIZZIh9AqutHpCgcXGKrWfY+Bgu2Y+HJT+ixGaEgy
# iluCaRXak7BfPe6MrGr/ad+Nndxl3UHOYS1lVu/n/nLG3uMle7S1BFvChVoXxgWm
