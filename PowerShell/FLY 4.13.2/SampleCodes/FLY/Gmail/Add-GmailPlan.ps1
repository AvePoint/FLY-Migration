...
try
{
    ...
    #   $OnlineConnectionOption = New-ExchangeOnlineConnectionOptionObject -BasicCredential $BasicCredential
	...
	# For example:
	# $SourceMapping1 = 'user@contosogsuite.com'
    # $SourceMapping1 = 'user@contosogsuite.com\folder'
	# $SourceMapping2 = 'resource@contosogsuite.com'
    $SourceMapping1 = ''
	$SourceMapping2 = ''

    # For example: 
	# User Mailbox: $DestinationMapping1 = New-GmailMigrationExchangeMailboxObject -Mailbox 'user@contoso.onmicrosoft.com' -MailboxType UserMailbox
    # Archive Mailbox: $DestinationMapping1 = New-GmailMigrationExchangeMailboxObject -Mailbox 'user@contoso.onmicrosoft.com(In-Place Archive Mailbox)' -MailboxType ArchiveMailbox
    # Resource Mailbox: $DestinationMapping1 = New-GmailMigrationExchangeMailboxObject -Mailbox 'resource@contoso.onmicrosoft.com' -MailboxType ResourceMailbox
    # Shared Mailbox: $DestinationMapping1 = New-GmailMigrationExchangeMailboxObject -Mailbox 'shared@contoso.onmicrosoft.com' -MailboxType SharedMailbox
    $DestinationMapping1 = New-GmailMigrationExchangeMailboxObject -Mailbox '' -MailboxType UserMailbox
	$DestinationMapping2 = New-GmailMigrationExchangeMailboxObject -Mailbox '' -MailboxType SharedMailbox

    # For example: 
	# UserMailbox: $MappingContent1 = New-GmailMappingContentObject -Mailbox $SourceMapping1  -MailboxType UserMailbox -Destination $Destination -MigrateArchiveFolder
	# ResourceMailbox: $MappingContent2 = New-GmailMappingContentObject -Mailbox $SourceMapping2  -MailboxType ResourceMailbox -Destination $Destination
    $MappingContent1 = New-GmailMappingContentObject -Mailbox $SourceMapping1  -MailboxType UserMailbox  -Destination $DestinationMapping1  -MigrateArchiveFolder
	$MappingContent2 = New-GmailMappingContentObject -Mailbox $SourceMapping2 -MailboxType ResourceMailbox  -Destination $DestinationMapping2 

    # Verify the mailboxes exist and are available for migration.
    # For example: $VerifyMapping = $True/$False
    $VerifyMapping = $False;

    $Mappings = New-GmailMappingObject -SourceConnectionId $GmailConnectionId -Destination $DestinationConnection -Contents @($MappingContent1,$MappingContent2) -VerifyMapping:$VerifyMapping
    ...
    $PlanSettings = New-GmailPlanSettingsObject -DisplayName $PlanName -Schedule $Schedule -PolicyId $Policy -DatabaseId $Database -SynchronizeDeletion -MigrateFilters -PlanGroups $PlanGroups
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDOJalO6xogzIX3
# DjJbpcp904EdLqvZ39fIX/UKZTRoT6CCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiB