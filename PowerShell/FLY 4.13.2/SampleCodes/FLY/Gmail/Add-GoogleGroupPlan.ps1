...
try
{
    # For example: $GmailConnectionId = 'Gmail connection Id or Name'
    $GmailConnectionId = ''
    ...
    #   $DestinationConnOption = New-ExchangeOnPremisesConnectionOptionObject -ExchangeServerOption $ExchangeServer  -BasicCredential $Credential 
    #   $DestinationConnection = New-ExchangeConnectionObject -OnPremisesConnectionOption $DestinationConnOption

    # Exchange online connection example
    # online service account:
    #   $Credenttial = New-BasicCredentialObject -Username 'Exchange Online username' -Password 'Exchange Online password'
    #   $OnlineConnectionOption = New-ExchangeOnlineConnectionOptionObject -BasicCredential $Credenttial
    #   $DestinationConnection  = New-ExchangeConnectionObject -OnlineConnectionOption $OnlineConnectionOption
    # online connection:
    ...
    # For example: 
    # $SourceMapping1 = 'GoogleGroup1@gmail.com'
    # $SourceMapping2 = 'GoogleGroup2@gmail.com'
    # $SourceMapping3 = 'GoogleGroup3@gmail.com'
    $SourceMapping1 = ''
	$SourceMapping2 = ''
    $SourceMapping3 = ''

    # For example: 
    # Microsoft 365 Group: $DestinationMapping1 = New-GoogleGroupMigrationExchangeMailboxObject -Mailbox 'Group@contoso.onmicrosoft.com' -MailboxType Microsoft365Group
    # Distribution Group: $DestinationMapping2 = New-GoogleGroupMigrationExchangeMailboxObject -Mailbox 'Distribution@contoso.onmicrosoft.com' -MailboxType DistributionGroup
    # Security Group: $DestinationMapping3 = New-GoogleGroupMigrationExchangeMailboxObject -Mailbox 'SecurityGroup@contoso.onmicrosoft.com' -MailboxType SecurityGroup
    $DestinationMapping1 = New-GoogleGroupMigrationExchangeMailboxObject -Mailbox '' -MailboxType Microsoft365Group
	$DestinationMapping2 = New-GoogleGroupMigrationExchangeMailboxObject -Mailbox '' -MailboxType DistributionGroup
    $DestinationMapping3 = New-GoogleGroupMigrationExchangeMailboxObject -Mailbox '' -MailboxType SecurityGroup

    $MappingContent1 = New-GoogleGroupMappingContentObject -Mailbox $SourceMapping1 -Destination $DestinationMapping1
	$MappingContent2 = New-GoogleGroupMappingContentObject -Mailbox $SourceMapping2 -Destination $DestinationMapping2
    $MappingContent3 = New-GoogleGroupMappingContentObject -Mailbox $SourceMapping3 -Destination $DestinationMapping3

    # Verify the mailboxes exist and are available for migration.
    # For example: $VerifyMapping = $True/$False
    $VerifyMapping = $False;

    $Mappings = New-GoogleGroupMappingObject -SourceConnectionId $GmailConnectionId -Destination $DestinationConnection -Contents @($MappingContent1,$MappingContent2,$MappingContent3) -VerifyMapping:$VerifyMapping
    ...
    # For example: $Schedule = New-SimpleScheduleObject -IntervalType Once/Daily -StartTime ([System.DateTime]::Now).AddMinutes(2).ToString("o")
    ...
    $PlanSettings = New-GoogleGroupPlanSettingsObject -DisplayName $PlanName -Schedule $Schedule -PolicyId $Policy -DatabaseId $Database -PlanGroups $PlanGroups
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCvHboMNiRCjVp0
# R/ZMccxWB/B1oBL2CdZkDpMUEwUkKKCCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiB