...
try
{
    # For example: $ScopeName = 'Scope Name'
    $ScopeName = ''

    # For example: $Database = 'Database Id or Database Name(Database Server)'
    $Database = ''

    # For example: $Connection = 'Connection Id or Name'
    $Connection = ''

    # For example: $ScanItemMode = 'Detailed or Quick' 
    $ScanItemMode = ''

    # For example: 
    # User Mailbox: $Mapping1 = New-ExchangePreMappingObject -EmailAddress 'user@contoso.onmicrosoft.com' -MailboxType UserMailbox
	# Archive Mailbox: $Mapping1 = New-ExchangePreMappingObject -EmailAddress 'user@contoso.onmicrosoft.com(In-Place Archive Mailbox)' -MailboxType ArchiveMailbox
    # Shared Mailbox: $Mapping1 = New-ExchangePreMappingObject -EmailAddress 'shared@contoso.onmicrosoft.com' -MailboxType SharedMailbox
	# Resource Mailbox: $Mapping1 = New-ExchangePreMappingObject -EmailAddress 'resource@contoso.onmicrosoft.com' -MailboxType ResourceMailbox
	# Microsoft 365 Group Mailbox: $Mapping1 = New-ExchangePreMappingObject -EmailAddress 'group@contoso.onmicrosoft.com' -MailboxType Microsoft365GroupMailbox
    # Scheduling Mailbox: $Mapping1 = New-ExchangePreMappingObject -EmailAddress 'user@contoso.onmicrosoft.com' -MailboxType SchedulingMailbox
    $Mapping1 = New-ExchangePreMappingObject -EmailAddress '' -MailboxType UserMailbox
    $Mapping2 = New-ExchangePreMappingObject -EmailAddress '' -MailboxType ResourceMailbox

    $ImportMappings = @($Mapping1,$Mapping2)

    $Scope = New-ExchangePreMigrationObject -ScopeName $ScopeName -Connection $Connection -ImportMappings $ImportMappings -MigrationDB $Database -ScanItemMode $ScanItemMode

    $Response = Add-ExchangePreMigration -APIKey $ApiKey -BaseUri $BaseUri -Scope $Scope
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB4cxVzJE9iTOq3
# f+cov3APdK5vzPHrSwbmyfEvq9vpxqCCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCBcKGXXnyxR8RkA74Cy/kO4KmNG+f/fbtak2cHIMqfzPTANBgkq
# hkiG9w0BAQEFAASCAYDGlKBWJdaKHnCwbryO90AAEHlTtPkEthfXxmL/EmqLar7p
# VtjFp8MDOvXHUBnL1+lktPkGtf28A2cbFSE85t2T2jGRpXMu28n1b+7aXbvcHy1P
# 5bhoJrHa8pyPis49EFascq8kftCB/fU5Rxhf0neSnfK4X0k0FU9yEbOyjspM2io6
# k1b7nQu2Dj5UDMXQqmcWNeT2zaa6x6/3F3fzHvhz7J82oPhTk/Yz7AgaUl8psz9+
# 5hR+V3EjtMFoI/qXV1hDTaY8R8H3Wrwx3E4kfJ7cOeaK0ZOXAGCyaaTDuTXq9wAh
# k60n37SksBgtwoYqOyv8f2fJn0kpabC3EZssy0dLqyLBq+X82j5St/XgUZ/qY7Rf
# Qpn/McD7SIA2g4rYWsdXOjxzvI5M9s0SCeRhkNAnlNvJkQZ0SCE8eFpLnIA8V4Lu
# TTmj9rgPXovvkAwWY1hp4POIxIqknfu3xdKNlKtaEcy/WI7Q6A1vn2Y+DftbZ1Xk
# /hr38NGO/X/B+zsEJrShghc5MIIXNQYKKwYBBAGCNwMDATGCFyUwghchBgkqhkiG
# 9w0BBwKgghcSMIIXDgIBAzEPMA0GCWCGSAFlAwQCAQUAMHcGCyqGSIb3DQEJEAEE
# oGgEZjBkAgEBBglghkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgXvtrckGHRbYP
# 2WpD8jw+lIV6FGF8Df2Or01Te3XI4BMCECzKO75kVitUKFcfVWZYKtQYDzIwMjUw
...
# BgkqhkiG9w0BCQQxIgQg61Oc3HNt7F4Ai0X8xrD0tEB4Z7vlJH+m8AOm/IJQb4Qw
# NwYLKoZIhvcNAQkQAi8xKDAmMCQwIgQgdnafqPJjLx9DCzojMK7WVnX+13PbBdZl
# uQWTmEOPmtswDQYJKoZIhvcNAQEBBQAEggIAFGuGMzI5VvxDL1ioOUTcWsDlImbf
# uOjBmUca7fDRMfnB3eoD8k5p/zU+JgeBki3mLXiD12Guy2cws3DRp9tSkmZgNJ5r
# su0mZpbXN6wjQZMayNbxq1zHm9NVqidiGBCOsrTiNX2YKcW8TQrZ1AcxD319iTO3
# MG6Uuyic6X+6L/fRabcf4l+JN2nB8udM57ubZVyq3nkS7n8GIpTlV7eHhAaUMsha
# HtMx2PCL9ttVZe27yZls4zCr2iq053h3G2h48CVXJosyGZJ1AQOATgS+DApzdHiF
# BqnCHMKj0uLi3rMdJ9XKIbANowhSH9WJV/nTRCg9RsWm+oKdGSkWfimkRslHK9z4
# DZb5NIxzpXFVjm75mxwL2s2vFtn90mQkDOiOjrOhGXuEc53yhRZ7XlLYigImNQEh
# togbqAky7IKNxbtUYczywo2zPEn1Ceb5i2kUpcrvtYypNBLieZN0avJJMugAE/U8
# VYmJlh0mkDleHO60YiD32Dnw80rKXE8O5zU0hr3Psj4Z2/VQTS8TFA7OkjTOIh0c
# LTOEJR0oHL2Q97n/jgmEYWsPw4r2tQB0YTLeE+mrQ9oBxxsAD85t+oKobwrYh81o
# ZEbVrTBrmQiX9V4pCanIswvWvB8RTmG/q4B9p446iu4h+eihSz+QyNRrMIB3jpKA
# 1djD4KrORflfdGY=
