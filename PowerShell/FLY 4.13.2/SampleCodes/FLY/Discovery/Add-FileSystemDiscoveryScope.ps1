...
try
{
    ...
    # For example: $Credential =  New-CredentialObject -Username 'Service Account' -Password 'Account Passwrod'
    $Credential =  New-CredentialObject -Username '' -Password ''

    # For example: $ImportMappings = @('\\admin-PC\c$\data','\\admin-PC\shared folder')
    $ImportMappings = @()
    ...
    # For example:
    # If the $DestinationSharePointLibraryorFolderURL = '', the url value will be set to the default url 'https://contoso.sharepoint.com/sites/sitename/subsite/libraryname'
    # $DestinationSharePointLibraryorFolderURL = 'Custom Url'
    $DestinationSharePointLibraryorFolderURL = ''

    # For example: $Advanced = New-FileSystemAdvancedObject -DestinationVersion SharePoint2013/SharePoint2016/SharePoint2019/SharePointOnline -Agents $Agents -ScanFolderSizeAndItemCount -SharePointLibraryOrFolderUrl $DestinationSharePointLibraryorFolderURL
    $Advanced = New-FileSystemAdvancedObject -DestinationVersion SharePointOnline -Agents $Agents -ScanFolderSizeAndItemCount -SharePointLibraryOrFolderUrl $DestinationSharePointLibraryorFolderURL

    $Scope = New-FSPreMigrationObject -ScopeName $ScopeName -Credential $Credential -ImportMappings $ImportMappings -Advanced $Advanced -MigrationDB $Database 
    $Response = Add-FileSystemPreMigration -APIKey $ApiKey -BaseUri $BaseUri -Scope $Scope
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAH44Z/KI3ItXrH
# LDQ1MAC5O/ActMxEgiam2KUPrDanj6CCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCAKKlal7keqQxwTdtsKmzlunuJg0ggVQXqdMPB4ojJPGTANBgkq
# hkiG9w0BAQEFAASCAYBIXJBMApUgWAKSzeYAGWYXeU2mHLInqUNcC0DdwxaV2WhM
# Yg3dRxX3w1ZVAtfhrUeLe8VCSBIsNBzkDARl7kfq6/CpXbo1RZtN9SgqcjH7Hh7V
# kqIMyuIaUmMJIlw8pwPSHVVEdbDL6ivfExD4KjA8daBHXpgEg0wD+hdb9MewO6+H
# 3w1YWVPhsC0SyPvpXK9JpfKDppYsR6wwyP3UohkHX3k8G4N8sDGj5tSOz52KphOy
# EGXraeAVhkJnWBprLfiIum8yiyGroJ7TtjIH6h8uTLGnJSbz/YO42PB//YjkM5++
# h7CB64vB/G9WtexAayneaf2nSbY98AGEIUL3oEdGtxv3uGWLyU9+nibEveNDGPnC
# HzjxSyJHIxKwon/Bg5LO9p9JxhZDGf3Ae1SoVFoUmCwcrAH0ZAWTuiujI8Qn+yhb
# RQlnUFl3WVIjp8Zne1tdgjnNcQKp3azdO3NrwggNUgTrOA6dFWSYh+58wTH4jlLr
# MeQCAlx/wDMiwNSKuoyhghc5MIIXNQYKKwYBBAGCNwMDATGCFyUwghchBgkqhkiG
# 9w0BBwKgghcSMIIXDgIBAzEPMA0GCWCGSAFlAwQCAQUAMHcGCyqGSIb3DQEJEAEE
# oGgEZjBkAgEBBglghkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgXpJzXbN7Wla/
# 4cgfzXtOBYqKTSyx0MYZpKMIvC1IEOoCEAefUlvvfY6fsjTeAap8G1IYDzIwMjUw
...
# BgkqhkiG9w0BCQQxIgQgSccwE8wJl7zwX74MOYEU2s6Dz2J2+S6j1LR2+AFHr7Yw
# NwYLKoZIhvcNAQkQAi8xKDAmMCQwIgQgdnafqPJjLx9DCzojMK7WVnX+13PbBdZl
# uQWTmEOPmtswDQYJKoZIhvcNAQEBBQAEggIAo3A+cxdLqJYDHWUYr7WOQBP+G/Bz
# fJlvObir+A7Y/Fae2QiJXirILxeXChDSV4NCHfJtEGpXH4v2ycAvc6VWwQrT5Cwy
# rm5upnszPHLFpR5ISfcwc1I2vA0S6/R7oNps+I04mg1mwrbOs+aJvhSNiYg5TmR8
# 9dkVxckqsx1YngJXeJsA4pbqo2rN87iB/32Sjofu/hrAp4nz+TQKbyiJvy1GzXjM
# ERLz1a+jbAb6lwwDncSi2yu+YWy7EfaMO0steaw59c9LeOoqUgZ2r74VketVEcux
# NIMDbFNwpHuRK/mom2T1GqE0KpTMX2RZ9+cta/D01rQxDIB/SbsOIVvqx4anzUNH
# egR4x/y6jVrCw+KNF9pOMmyl1i9Vj98cAMl5ancPCYjMy2NJMe8XZMDpUAMddPCH
# 4mzY7WzLo869A02PjECfSAM0gl2cb4pl4inD89mlcijYPXiTT5PH5NmwDsvRLTWF
# ufc3d4ciQupj2INiynI/S4m1aS55PrNSEi8Zj6LKvrW1TKrgG0ckIt3Li8X3EFOQ
# muAq++bEWIWJj6ZsV5wJVF3K6sQOi4HOjDg5MfmZ2Vx7blKS/9pJs0p62AnQBl8X
# lRiwQYtQl7hRTeHWowd31eYR0HaTSd0VyKlyRQKiFgmMWIJA1215uEAXdv94tgyH
# gAQsuJT1v/dbC8o=
