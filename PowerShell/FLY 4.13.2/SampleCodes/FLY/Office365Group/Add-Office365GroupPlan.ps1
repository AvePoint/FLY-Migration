...
try
{
    ...
    # For example: 
    # $SourceMapping1 = New-Office365GroupObject -Mailbox 'Group1@contoso.onmicrosoft.com'
    # $SourceMapping2 = New-Office365GroupObject -Mailbox 'Group2@contoso.onmicrosoft.com'
    $SourceMapping1 = New-Office365GroupObject -Mailbox ''
    $SourceMapping2 = New-Office365GroupObject -Mailbox ''

    # For example:
    # $DestinationMapping1 = New-Office365GroupObject -Name 'group name1' -Mailbox 'Group@contoso01.onmicrosoft.com'
    # $DestinationMapping2 = New-Office365GroupObject -Name 'group name2' -Mailbox 'Group@contoso02.onmicrosoft.com'
    $DestinationMapping1 = New-Office365GroupObject -Name '' -Mailbox ''
    $DestinationMapping2 = New-Office365GroupObject -Name '' -Mailbox ''

    $MappingContent1 = New-Office365GroupMappingContentObject -Source $SourceMapping1 -Destination $DestinationMapping1
    $MappingContent2 = New-Office365GroupMappingContentObject -Source $SourceMapping2 -Destination $DestinationMapping2
    ...
    # For example:
    # Select "Migrate the whole team site of the Group" option: $TeamSiteScope = $False; 
    # Select "Only migrate files in the default Documents library of Group's team site" option: $TeamSiteScope = $True;
    $TeamSiteScope = $False;
    ...
    # For example: $MultigeoTenant = New-MultigeoTenantObject -MultigeoTenant:$False/$True -Location 'location code'
    ...
    # For example: the following '-MigrateGroupPlanner' parameter indicates that Migrate Group Planner is checked. If you do not want to check, please do not fill in the corresponding parameters.
    # If you do not want to set the Schedule, please remove the '-Schedule $Schedule ' parameter below.
    $PlanSettings = New-Office365GroupPlanSettingsObject -DisplayName $PlanName -PolicyId $Policy -DatabaseId $Database -Schedule $Schedule -PlanGroups $PlanGroups -OnlyMigrateDocumentsLibrary:$TeamSiteScope -MigrateGroupPlanner -MultigeoTenant $MultigeoTenant -Migrate365GroupMailbox
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDLfApLeE6JHtNN
# S19oZ2V338YoRq6U+jPIZzWRgHI2o6CCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCBAOeWT+7nz/EQnlFIUxus8v0erbQSghWgdU/LeBw4DpjANBgkq
# hkiG9w0BAQEFAASCAYCGY7927HLNdnZJ8nQYmfgC9t0Tu4GRLMWthrbz3buFQd6+
# vcZZTrxQGos+Ook0Qx6BoC2BXEWW1el6f/0rP7hYG1B98vncCo2F0BgJJG2d3UDK
# 3/Q1hOxhT3ejHlEhsvtYQcQ0+7StIChcNuO6cKcRY3EJBMU6H/AB4KazV3Vr6aCc
# LaQIqEOW3sW25K7YHj8pskMxNLVKGaMrBFsDZVSAZ3iN6+vImjf6EFsAiLIXN6OF
