...
try
{
    # For example: $BoxConnection ='Box Connection Id or Name'
	$BoxConnection = ''
    ...
    # For example: 
	# $SourceMapping1 = New-BoxObject -Path 'user@contoso.com' -Level Folder
	# $SourceMapping2 = New-BoxObject -Path 'user@contoso.com\folder' -Level Folder
	$SourceMapping1 = New-BoxObject -Path '' -Level Folder
	$SourceMapping2 = New-BoxObject -Path '' -Level Folder

    # For example: 
    # OneDrive:
    #  Library：$DestinationMapping1 = New-BoxMigrationSharePointObject -Url 'user@contoso.onmicrosoft.com' -Level Library
	#  Folder: $DestinationMapping2 = New-BoxMigrationSharePointObject -Url 'user@contoso.onmicrosoft.com\folder' -Level Folder
    # SharePoint:
	#  Library：$DestinationMapping1 = New-BoxMigrationSharePointObject -Url 'https://contoso/sites/sc/library' -Level Library
	#  Folder: $DestinationMapping2 = New-BoxMigrationSharePointObject -Url 'https://contoso/sites/sc/sub/list/folder' -Level Folder
	$DestinationMapping1 = New-BoxMigrationSharePointObject -Url '' -Level Library
	$DestinationMapping2 = New-BoxMigrationSharePointObject -Url '' -Level Folder

    # For example: $MappingContent1 = New-BoxMappingContentObject -Source $Source1 -Destination $Destination1 -Method Combine/Attach
	$MappingContent1 = New-BoxMappingContentObject -Source $SourceMapping1 -Destination $DestinationMapping1 -Method Combine
	$MappingContent2 = New-BoxMappingContentObject -Source $SourceMapping2 -Destination $DestinationMapping2 -Method Attach

	$Mappings = New-BoxPlanMappingObject -SourceConnectionId $BoxConnection -DestinationConnectionId $DestinationConnection -Contents @($MappingContent1, $MappingContent2) -DestinationType $DestinationType
    ...
    # For example: $Policy = 'Migration Policy Id or Name'
	$Policy = ''

    # For example: $Database = 'Database Id' or $Database = 'Database Name(Database Server)'
	$Database = ''
	...
	$PlanSettings = New-BoxPlanSettingsObject -DisplayName $PlanName -Schedule $Schedule -PolicyId $Policy -DatabaseId $Database -MigrateVersions:$MigrateVersions -MigrateLastVersion $MigrateLastVersion -PlanGroups $PlanGroups
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDWrlu0TDy9ep6J
# XvjZdrv+BQeEfnZMgoOPdWMwYdhZI6CCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCAkZNbY+m9cz7cKCyH6l/k90OVPqTSztFlFtQZ/ge+BbTANBgkq
# hkiG9w0BAQEFAASCAYDBsMK1K13TVhYo+pdWICwD+mUPH3AqI4rDbcF27gxIhACX
# PJUCP4B0nYMLPpxNbr0ADfNk288aqxQTfyORlB/ITEYcdyddHjt5uYkifKNyjmoc
# ZjSsAuNFTizvzJpseVvYPesCdMDL0ytJufWKBU1T1BXYybNr+r9SHDuHiFKb2f+x
