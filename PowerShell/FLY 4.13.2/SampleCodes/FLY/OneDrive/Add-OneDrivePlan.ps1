...
try
{
    # For example: $SourceConnection = 'OneDrive connection Name'
    $SourceConnection = ''

    # For example: $DestinationConnection = 'OneDrive connection Name'
    $DestinationConnection = ''

    # For example: 
    # $SourceMapping1 = New-OneDriveObject -User 'user1@contoso.onmicrosoft.com'
    # $SourceMapping2 = New-OneDriveObject -User 'user2@contoso.onmicrosoft.com'
    # $SourceMapping3 = New-OneDriveObject -User 'user3@contoso.onmicrosoft.com\folder'
    $SourceMapping1 = New-OneDriveObject -User ''
    $SourceMapping2 = New-OneDriveObject -User ''
    $SourceMapping3 = New-OneDriveObject -User ''

    # For example: 
    # $DestinationMapping1 = New-OneDriveObject -User 'user1@contoso.onmicrosoft.com'
    # $DestinationMapping2 = New-OneDriveObject -User 'user2@contoso.onmicrosoft.com'
    # $DestinationMapping3 = New-OneDriveObject -User 'user3@contoso.onmicrosoft.com\folder'
    $DestinationMapping1 = New-OneDriveObject -User ''
	$DestinationMapping2 = New-OneDriveObject -User ''
    $DestinationMapping3 = New-OneDriveObject -User ''

    # Verify the user exist and are available for migration.
    # For example: $VerifyMapping = $True/$False
    $VerifyMapping = $False;

	# For example: MappingContent1 = New-OneDriveMappingContentObject -Source $Source -Destination $Destination
    $MappingContent1 = New-OneDriveMappingContentObject -Source $SourceMapping1 -Destination $DestinationMapping1 
	$MappingContent2 = New-OneDriveMappingContentObject -Source $SourceMapping2 -Destination $DestinationMapping2
	$MappingContent3 = New-OneDriveMappingContentObject -Source $SourceMapping3 -Destination $DestinationMapping3


    $Mappings = New-OneDriveMappingObject -SourceConnectionId $SourceConnection -DestinationConnectionId $DestinationConnection -Contents @($MappingContent1,$MappingContent2,$MappingContent3) -VerifyMapping:$VerifyMapping
    ...
    # -MigrationMode HighSpeed\CSOM
    #    HighSpeed: The High Speed method is recommended for migrations. Large volumes of files and folders can be quickly migrated into OneDrive for Business. Content is first sent to Azure storage, and then imported into Microsoft 365. No need to bring your own Azure - we use the default Microsoft 365 storage. If you have additional security requirements, configure your own Azure storage location.
    #    CSOM: The Classic method is only recommended for limited migration scopes, or for customers in data centers that do not have access to Microsoft Migration APIs. This method migrates data directly to Microsoft 365 using the client-side object model (CSOM). All sites, site structure, security, properties, and settings are migrated in addition to your content libraries.
    $PlanSettings = New-OneDrivePlanSettingsObject -MigrationMode HighSpeed -DatabaseId $Database -PolicyId $Policy -DisplayName $PlanName -Schedule $Schedule -PlanGroups $PlanGroups

    $Plan = New-OneDrivePlanObject -Settings $PlanSettings -Mappings $Mappings

    $Response = Add-OneDrivePlan -APIKey $ApiKey -BaseUri $BaseUri -PlanSettings $Plan
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBFUaDBHDGcVV5B
# t3p7jLZy4e1pFPs3EYnnaWjXHEoTaqCCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiB