...
try{
	...
	$connections  = Import-Csv -Path $NewFilePath
		...
		foreach($connection in $connections)
		{
			...
			$sourceConnection = CheckData -data $connection."Source Connection"
			
			$destConnection =  CheckData -data $connection."Destination Connection"

			foreach($csv in $Csvs)
			{
				$mappings = Import-Csv -Path $CSVFilePath"\$csv" -Encoding UTF8 -Header "Migrate From","Migrate To"
				...
				{
					if($mapping."Migrate From" -eq "Migrate From")
					{
						continue;
					}
					$Source = New-OneDriveObject -User $mapping."Migrate From"

					$Destination = New-OneDriveObject -User $mapping."Migrate To"

					$MappingContent = New-OneDriveMappingContentObject -Source $Source -Destination $Destination
                    Write-Host($mapping."Method")
					$mappingList += $MappingContent
				}
			   
				$Mappings = New-OneDriveMappingObject -SourceConnectionId $sourceConnection -DestinationConnectionId $destConnection -Contents $mappingList -VerifyMapping:$VerifyMapping
				...
				$PlanSettings = New-OneDrivePlanSettingsObject -MigrationMode HighSpeed -DatabaseId $Database -PolicyId $Policy -DisplayName $PlanNameLabel -Schedule $Schedule -PlanGroups $PlanGroups
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDg4UXFC7jQMDgH
# 6yiUwbi4Ed23QpfkJ7gyRl0rtroQrqCCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCDW5Yen9NMkPkqOhxt/sAnNCbICb+OyRwVyrOA8geV+gjANBgkq
# hkiG9w0BAQEFAASCAYAruFNhBpRU1GBXkYc9dcHo3Cf+YYGcYHmV0LgNqulZkuNB
# bkMwmUmb+5O84aD+9a6GD7kG35OCxVg5pKjGV0+VWEG1a/WHMTfIf+bBGZhNqoYI
# UQ1MXG1/3ci0XxdECyeYkSnp3smY5xutHXmxpe/KVpLY8sNfSCoE8if1IzeOttts
# 2nl8Tlecl7fK/MsANUBpRQ6UTy1MWFZv4Mu9GXjsltDxkiV/+XmPFY479xwhyS14
# QITKVqamjcGIcgwzwvl/CbeOLYb6NDhTk2oFZTDSuzZUmSa7pYww2nuAtdNRE0LB
# YYCJbduhW1MEqGxAVLxRtGyvZ1KsLnSrI7gm9HELGdpTYPMAZu8b1bcjX2/GC+mQ
# J1NJqS9+3TI+zp6HelDKqP0SSWoTGO2hiiEfqnQUw1XDm5CRQj40ZzkoD0634jql
# Tggdv8Rt4jOlgkaih3aOfaR8A/KLtr/HsQMIaE57CrYzg8neKtqkqbs+X5A+QuET
# OSnXLs65kh+/E0TKg5+hghc6MIIXNgYKKwYBBAGCNwMDATGCFyYwghciBgkqhkiG
# 9w0BBwKgghcTMIIXDwIBAzEPMA0GCWCGSAFlAwQCAQUAMHgGCyqGSIb3DQEJEAEE
# oGkEZzBlAgEBBglghkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgvsrfJ8XUogbT
# FtrKjnTHstTnk292QpHxZz3UWavkpLYCEQDtXekRMT2gr8Eif1cKXYs3GA8yMDI1
# MDUyMzA3MTgzNVqgghMDMIIGvDCCBKSgAwIBAgIQC65mvFq6f5WHxvnpBOMzBDAN
...
# NVowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQU29OF7mLb0j575PZxSFCHJNWGW0Uw
# LwYJKoZIhvcNAQkEMSIEIEkZI74klF7YIVLLWmutWc1bDRAwar1X/4/LkX24Qi0i
# MDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEIHZ2n6jyYy8fQws6IzCu1lZ1/tdz2wXW
# ZbkFk5hDj5rbMA0GCSqGSIb3DQEBAQUABIICAF43Z1JT6/9Xcc4mLbHb6uXeuIY6
# 4FfPeOktXiCMR7HbfL6kwj2BRMKa1ZLk0+ePLP+o/3eDGenjoUI/U2Y5DDPIZAxx
# G2xUo6qkNllmKAcdgUht3LwqLzNOSXQ7Q/BtS0uW7m+4iIuTFgApPFhQu07X81/a
# 4lxv9ovHBvrIG2IJxHL60aEsNPi9v2FEBQJJTFj9zPMJHQtgwvRqpe90EZ4YWisJ
# pVANHZyj+JW/WbyKaWyAnRZyaQbWC+Kwkcsf95iguEzcclmoNCoQ3yircAUg/Ru5
# MYaXDT6HjWk0Fez2Rf1KXWxqW2LRA0DVGEy0eocboijNqhl8Nf0K/KNgEbLWq0vZ
# bTAGJr6i8lmSFjnpc1H05AUqAwl2HItnSvegT/jfRA3L9EMalMdEvIP3hIG7+C2d
# 2xOV4M9FU41XNhUjLkv56Uto88S61w7QfPW1gn8SijxBgAQootIm5PXlo/RG5et8
# JY3C2g9I3i0UzPMgCFxfFf2iN7/42fFgk4NbOOwlfel+6/albAOKGiIxgnMHyPaY
# Njth2wOsSKIrpTlf355DXx0NZvTjbd0TkcB2fOCqfMXVzr6mkH4qlCCoUYXaoorH
# TDDvJe77ABm/cXL1Lq67kItct8KhVwKTyXgl46Cpa4n1gF7tmOl7TZ1K4LUeaBhJ
# XreGv0PnaQ8kSwzN
