...
try
{
    # For example: $GoogleDriveConnection = 'Google Drive connection Id or Name'
    $GoogleDriveConnection = ''

    # For example: $DestinationType = 'OneDrive'/'SharePoint'
    $DestinationType = 'OneDrive'

    # For example: $DestinationConnection = 'connection Name'
    $DestinationConnection = ''

    # For example: 
    # $SourceMapping1 = New-GoogleDriveObject -Level Folder -Path 'user@contoso.com\folder'
    # $SourceMapping2 = New-GoogleDriveObject -Level Folder -Path 'Shared Drive\folder'
    $SourceMapping1 = New-GoogleDriveObject -Level Folder -Path ''
    $SourceMapping2 = New-GoogleDriveObject -Level Folder -Path ''
	
	

    # For example:
    # OneDrive:
    #  Library: $Destination1 = New-GoogleDriveMigrationSharePointObject -Level Library -Url user@contoso.onmicrosoft.com
    #  Folder: $Destination2 = New-GoogleDriveMigrationSharePointObject -Level Folder -Url user@contoso.onmicrosoft.com\folder
    # SharePoint:
    #  Library:  $Destination1 = New-GoogleDriveMigrationSharePointObject -Level Library -Url 'https://contoso-my.sharepoint.com/personal/user_contoso_onmicrosoft_com/library'
    #  Folder:  $Destination2 = New-GoogleDriveMigrationSharePointObject -Level Folder -Url 'https://contoso/sites/sc/sub/list/folder'
    $DestinationMapping1 = New-GoogleDriveMigrationSharePointObject -Level Library -Url ''
    $DestinationMapping2 = New-GoogleDriveMigrationSharePointObject -Level Folder -Url ''

    $MappingContent1 = New-GoogleDriveMappingContentObject -Source $SourceMapping1 -Destination $DestinationMapping1 -Method Combine
    $MappingContent2 = New-GoogleDriveMappingContentObject -Source $SourceMapping2 -Destination $DestinationMapping2 -Method Attach



    $Mappings = New-GoogleDriveMappingObject -SourceConnectionId $GoogleDriveConnection -DestinationConnectionId $DestinationConnection -Contents @($MappingContent1, $MappingContent2) -DestinationType $DestinationType
    ...
    # latest versions: $MigrateLastVersion = '10' 
	# The value of latest versions is a integer greater than zero and cannot exceed 50000.
    $MigrateLastVersion = ''
    ...
    $PlanSettings = New-GoogleDrivePlanSettingsObject -DisplayName $PlanName -Schedule $Schedule -DatabaseId $Database -PolicyId $Policy -PlanGroups $PlanGroups -MigrateVersions:$MigrateVersions -MigrateLastVersion $MigrateLastVersion
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBro3CMj0Ept74Y
# Pgrf05l4kJw+eEQuxrN705BsxYSD+aCCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiB