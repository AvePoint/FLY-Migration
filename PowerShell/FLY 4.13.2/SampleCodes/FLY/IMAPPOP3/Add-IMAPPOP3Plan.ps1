...
try
{
    ...
    # Imap Default Server Name：
    #   Gmail - imap.gmail.com
    #   Yahoo - imap.mail.yahoo.com
    #   OutLook - imap-mail.outlook.com
    # For example: $SourceConnection = New-IMAPPOP3ConnectionObject -Type Gmail/Outlook/Yahoo/Others -ServerName 'service name' -Port 993 -EnableSSL:$True/$False 
    # For example: $SourceConnection = New-IMAPPOP3ConnectionObject -Type Zimbra -ServerName 'service name' -Port 993 -EnableSSL:$True/$False -ApiPort 7070
    # ApiPort works only for Zimbra 
    $SourceConnection = New-IMAPPOP3ConnectionObject -Type Gmail -ServerName '' -Port 993 -EnableSSL:$True 
    ...
    # For example: 
    # $SourceMapping1 = New-IMAPPOP3MailBoxObject -Mailbox 'Outlook mailbox1' -Password 'Password'
    # $SourceMapping2 = New-IMAPPOP3MailBoxObject -Mailbox 'Outlook mailbox2' -Password 'Password'
	$SourceMapping1 = New-IMAPPOP3MailBoxObject -Mailbox '' -Password ''
    $SourceMapping2 = New-IMAPPOP3MailBoxObject -Mailbox '' -Password ''

    # For example:
    # Archived Mailbox: $DestinationMapping1 = New-ImapMigrationExchangeMailboxObject -Mailbox 'user@contoso.onmicrosoft.com(In-Place Archive Mailbox)' -MailboxType ArchivedMailbox
    # Resource Mailbox: $DestinationMapping1 = New-ImapMigrationExchangeMailboxObject -Mailbox 'resource@contoso.onmicrosoft.com' -MailboxType ResourceMailbox
    # Shared Mailbox: $DestinationMapping1 = New-ImapMigrationExchangeMailboxObject -Mailbox 'shared@contoso.onmicrosoft.com' -MailboxType SharedMailbox
    # User Mailbox: $DestinationMapping1 = New-ImapMigrationExchangeMailboxObject -Mailbox 'user@contoso.onmicrosoft.com' -MailboxType UserMailbox
    $DestinationMapping1 = New-ImapMigrationExchangeMailboxObject -Mailbox '' -MailboxType UserMailbox
    $DestinationMapping2 = New-ImapMigrationExchangeMailboxObject -Mailbox '' -MailboxType ResourceMailbox

    $MappingContent1 = New-IMAPPOP3MappingContentObject -Source $SourceMapping1 -DestinationMailbox $DestinationMapping1
	$MappingContent2 = New-IMAPPOP3MappingContentObject -Source $SourceMapping2 -DestinationMailbox $DestinationMapping2

    # Verify the mailboxes exist and are available for migration.
    # For example: $VerifyMapping = $True/$False
    $VerifyMapping = $False;

    $Mappings = New-IMAPPOP3MappingObject -Source $SourceConnection -Destination $DestinationConnection -Contents @($MappingContent1,$MappingContent2) -VerifyMapping:$VerifyMapping -SourceAgents $SourceAgents
    ...
    $PlanSettings = New-IMAPPOP3PlanSettingsObject -DisplayName $PlanName -Schedule $Schedule -PolicyId $Policy -DatabaseId $Database -SynchronizeDeletion -PlanGroups $PlanGroups
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDfnECDMFMPow3u
# aKDfcF6/NMF8Q3t7IJsoiAh6VDgXY6CCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiB