...
try
{
	...
	# If you need to change the plan name.
	# For example: $NewName = 'New name'
    $NewName = ''
	
	# Keep the original policy: $Policy = ''
	# Modify the policy:$Policy = 'Policy Name' or $Policy = 'Policy Id'
    $Policy = ''

    # Keep original Migration database : $Database = ''
	# Modify Migration Database: $Database = 'Database Id' or $Database = 'Database Name(Database Server)' 
	...
	# Modify Schedule: $Schedule = New-SimpleScheduleObject -IntervalType Once/Daily -StartTime ([Datetime]::Now).AddMinutes(2).ToString('o')
    ...
    # If you want to keep the advanced settings on the settings page, delete the corresponding parameters from the New-UpdateDropBoxPlanObject cmdlet. 
    # For example:
    # To keep the original 'Migrate LastVersion' setting, delete the ' -MigrateVersions:$MigrateVersions' and '-MigrateLastVersion $MigrateLastVersion ' parameters from the New-UpdateBoxPlanObject cmdlet.
    $data = New-UpdateDropBoxPlanObject -DisplayName $PlanName  -NewName $NewName -Policy $Policy -PlanGroups $PlanGroup -DisablePlanGroup:$DisablePlanGroup  -Schedule $Schedule -DisableSchedule:$DisableSchedule  -DatabaseId $Database  -DisableMigrationDatabase:$DisableMigrationDatabase  -MigrateVersions:$MigrateVersions -MigrateLastVersion $MigrateLastVersion

	$Response = Update-DropboxPlan -APIKey $ApiKey -BaseUri $BaseUri -Settings $data
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBhxTXrB4XtfnSL
# 0Y8FMJJWn12AQWpJPjmSUDhEbILyj6CCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCC3oxISonSax0srizP3As+hCpAvQSbDGtPspo9azvrsPDANBgkq
# hkiG9w0BAQEFAASCAYAOpeqWFlXLXHxogUH6gqD8ZNjbRmSBXs33ZKq1/USVvOFf
# +vj5M/je0deCaAV+1QnYNZcr816JgCWMRW6/xZnsUe8lnahjGMaxWPT9EdLZUtmk
# 87bi9/qnLHdPSs9tAJC+HX3coC2Kj1/9QulkadZNr5ghmNLEhA0icubjETHfPo7y
# 0FDSlzfhB4nPI7lUOQUU2mp5uLaw5C0rcQNsF+ccYPsSEW6UhwBC788Wi+8pfpWX
# Nyv+Q6J3bUepV0QJW/JEQIZLTwGHJjEw4ONyDn03qHZV7qcvzeCdlHrQxtj47NZY
# 31elPDvGe6JP/CyLhNTALGRvrrjz1TyrgqQOk66eFfkxrdZMpan9bqMEn0ydoezJ
# viC8x5yyAgc59dsYheIJwLOhfvz4MkkreA64EefVEjL36dLfjSiRx+0qfwfmo10y
# OlocX87P8ncnKHeBSAwF59enZzxvxe3v1QDyfFmB76hvU1OYgyzsrlscHbkuqNFm
# YYW9LzlvSE+XyApRcz6hghc6MIIXNgYKKwYBBAGCNwMDATGCFyYwghciBgkqhkiG
# 9w0BBwKgghcTMIIXDwIBAzEPMA0GCWCGSAFlAwQCAQUAMHgGCyqGSIb3DQEJEAEE
# oGkEZzBlAgEBBglghkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgZ4v2P7PC84El
# Drg5EFGLTTZH2afyyt2jUpURM6l7bqkCEQDAbz4PdTlpXx8K/+X/LbFnGA8yMDI1
# MDUyMzA3MTgyMFqgghMDMIIGvDCCBKSgAwIBAgIQC65mvFq6f5WHxvnpBOMzBDAN
...
# MFowKwYLKoZIhvcNAQkQAgwxHDAaMBgwFgQU29OF7mLb0j575PZxSFCHJNWGW0Uw
# LwYJKoZIhvcNAQkEMSIEIK8Xise8LCdjTQgxygvFwPcfwWkqfNrlKK6d7KvjPr10
# MDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEIHZ2n6jyYy8fQws6IzCu1lZ1/tdz2wXW
# ZbkFk5hDj5rbMA0GCSqGSIb3DQEBAQUABIICAC91TtHbGYr8LV7EY7wqknueBUIi
# UcvRuTyWc5Ng4sgYjXt6JppM6CXGHvnFng1mRxahz4sN0rjoIwrqwWJehUPVidsd
# vDT5AoJhKtJTEAWpUi/nnKovn0W3k5hzGx8DYMJzKXcSc9EsgUhGea0Ssy9n9ybr
# MG/pJweMt/w7+qcgZYfzGknWYRFhL2xeGCMRE2Pb1I/9Ig6FmkAsqueZYKrZ3qAL
# Yt1UozuOgM8+ckT28/wXju2K5GMgvjzvSVNA/uD0+meBa5UMlHxA95geWDKSHJYt
# OlelkN9x2wNnrpcsTFr01ExlkC3RSvfK0ukofzsRYXfWZP28qpx3GDIny+Ug4+4b
# swMG7FM5qRrci9GdV4DlBMjOLfkadGuuWvQAduyrZtKBcTzD74KlX7FW2dFT/gVw
# 21Is8htuCOKvTpwdJ6UWFoBDn0Xsp9U6cUYaSMexp/1ZHyKgllM3biXEyt3xPd+/
# cJQezEPb+fJlBzmF15l9XBH0SuwY3pCRzSCbvfdRZRnzuaAY6Ih5HxQihw+tolOS
# tsj4KfxVgDdqZbmLpBGEk4hAmyDb+pca4gfgqsrDpvGzKtxGDp1jlcbSKNeLi4Ya
# mD5eakc2pw24j4cqgEFpL1ophs80uQSjMSivHoUTBslBsBjayU3m3qtBwcylGEzs
# OIo1Yweu7/CYn0Nx
