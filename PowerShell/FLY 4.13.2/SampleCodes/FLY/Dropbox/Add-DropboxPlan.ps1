...
try
{
	# For example: $DropboxConnection = 'Dropbox Connection Id or Name'
	$DropboxConnection = ''
    ...
    # For example:
	# $SourceMapping1 = New-DropboxObject -Path 'user@contoso.com' -Level Folder
	# $SourceMapping2 = New-DropboxObject -Path 'user@contoso.com\folder' -Level Folder
	$SourceMapping1 = New-DropboxObject -Path '' -Level Folder
	$SourceMapping2 = New-DropboxObject -Path '' -Level Folder

    # For example:
    # OneDrive:
	# Library: $DestinationMapping1 = New-DropboxMigrationSharePointObject -Url 'user@contoso.onmicrosoft.com' -Level Library
	# Folder: $DestinationMapping2 = New-DropboxMigrationSharePointObject -Url 'user@contoso.onmicrosoft.com\folder' -Level Folder
    # SharePoint:
    # Library: $DestinationMapping1 = New-DropboxMigrationSharePointObject -Url 'https://contoso/sites/sc/library' -Level Library
	# Folder: $DestinationMapping2 = New-DropboxMigrationSharePointObject -Url 'https://contoso/sites/sc/sub/list/folder' -Level Folder
	$DestinationMapping1 = New-DropboxMigrationSharePointObject -Url '' -Level Library
	$DestinationMapping2 = New-DropboxMigrationSharePointObject -Url '' -Level Folder

    # For example: $MappingContent1 = New-DropboxMappingContentObject -Source $Source1 -Destination $Destination1 -Method Attach/Combine
	$MappingContent1 = New-DropboxMappingContentObject -Source $SourceMapping1 -Destination $DestinationMapping1 -Method Combine
	$MappingContent2 = New-DropboxMappingContentObject -Source $SourceMapping2 -Destination $DestinationMapping2 -Method Attach

	$Mappings = New-DropboxPlanMappingObject -SourceConnectionId $DropboxConnection -DestinationConnectionId $DestinationConnection -Contents @($MappingContent1, $MappingContent2) -DestinationType $DestinationType

    # For example: $PlanName = 'Plan Name'
	$PlanName = ''

	# For example: $PlanGoups=@('Group Id1 or Name1','Group Id2 or Name2')
	$PlanGoups = @()
    ...
    # For example: $MigrateVersions = $False/$True
	$MigrateVersions = $False
	# For example: 
    # All versions: $MigrateLastVersion = '' or $MigrateLastVersion = 'all' 
    # latest versions: $MigrateLastVersion = '10' #The value of latest versions is a integer greater than zero and cannot exceed 50000.
	$MigrateLastVersion = ''
	...
	$PlanSettings = New-DropboxPlanSettingsObject -DisplayName $PlanName -Schedule $Schedule -PolicyId $Policy -DatabaseId $Database  -PlanGroups $PlanGoups -MigrateVersions:$MigrateVersions -MigrateLastVersion $MigrateLastVersion
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB8ieRAENznDh/T
# 7gs0Nu3V6wEwZ/rM0aCw4uXGi83NXaCCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCCQJlOnxTbhL4zc1uGCG9B/1/3KelW1BmlG5Vfv5JR7wDANBgkq
# hkiG9w0BAQEFAASCAYDf7Tm+dhF2SsGNW4Unqn2Wu2eBdk4t+0CR9jIZz3FM7aNL
# qHClVgBuM0GhIJu6d52BXGF7h8Dyun4IN8XTkCX0bvvgPQ8zb/Hcof6/zaM7b57W
# zbGTSwh659K+vokigRX1ts3PrS2IApOwAkFqLaitRGFUvTtR5HkV1NjG598pIY0r
# i1o1S5ckgxMwZlTr24agivw3vzUN/oimpkMQtRZ6qE+iU8o4gPsOpFDMwQBY4cTA
# 3VC0KJ0hQXx15o3WrylCbSKjOqaqjZxoFT/27FQYTfC3LjafNJHjT6Q1rQ05DJU+
# 6GFz8hS2LKNXB+Z/gSWiHWqARNH+2KjxEFHQKoG64qzwzyyn2TVhlCA3jN5IEME4
# wp6hYGRXo52YB1Ogoipt4+mBOZXeOGsKkCbQHogaoQHSNDwtiOBFrjp31nsNgpm8
# dAATookJxm7gJUZ97I7ff5NHwLEcQ3TNTBK616LBXPpT6eU/ecU67nVbYTDn0pl1
# EpwMeTZiTIf6oitl1e+hghc6MIIXNgYKKwYBBAGCNwMDATGCFyYwghciBgkqhkiG
# 9w0BBwKgghcTMIIXDwIBAzEPMA0GCWCGSAFlAwQCAQUAMHgGCyqGSIb3DQEJEAEE
# oGkEZzBlAgEBBglghkgBhv1sBwEwMTANBglghkgBZQMEAgEFAAQgAy0329WEOhd6
# JrNJ3qwFqV7CxKDVHfsJHm/Eb0uT/ioCEQCdSJMxSTW7kfaJ9lRMesf0GA8yMDI1
# MDUyMzA3MTgxOVqgghMDMIIGvDCCBKSgAwIBAgIQC65mvFq6f5WHxvnpBOMzBDAN
...
# LwYJKoZIhvcNAQkEMSIEIDroGHzjiiRyvE5SmJ5AnSGmgJG7vPg4RShify+fXmFQ
# MDcGCyqGSIb3DQEJEAIvMSgwJjAkMCIEIHZ2n6jyYy8fQws6IzCu1lZ1/tdz2wXW
# ZbkFk5hDj5rbMA0GCSqGSIb3DQEBAQUABIICAJeNiUKgcLrqMCxlkw32im33Up1x
# 16CIAUYQTbOkKjy0TSeeDvWd1jEdHIcZcLU8eF714Y72iWgMdrmZfltvIsEGqHGW
# LG4/6p9rJ4EStdwE5FeqSbPnnIFtEkUVROmi89BuUQN0ZK0QTIhbdioXXV0Tbl3U
# ldo3DlAMKd1Ti1W34vGmocpsyG5kxEeJzhZWU0yGrtsBxjxM5P+pmerabFvVke4W
# RunePXY54zjnmQQqK+ZLliS23ruri7OTZHlPH20sQFrKsmqnMFx2zDijZkw7T4Vl
# eC5xk6IO+bxU+6ahs6Ha81jwl7l7MET17d50YlbDqL7L5svRwUBSL+t5EOx2LLBa
# rWQSk/p+0PcURiJ5py+ehNZP2lc6yoFJNwILumJsFk1nxzhkh4zTP14dwMFof0N8
# LeVaWTuFkK0xasxutL84xDgc/7jDQCilKqDd00oZIbDoqCpfdbw5/Ztp3ezCO/lf
# B4dH/atRLqPZ6tkvmS1XhRSr4yO2QbGylyqZB0PsuJm9OXdEqA9hCn7si9qgKktF
# z0IRcW7SjBacAz5z/0sAOZ6bmVua/EaDDyCQOk3cwThMmq+4F2CoSZSiXZF0+ycv
# 5jjIO96HT33anhlQGkIcE6Zn7SaT1N1k9Q2aT2hbSArLCGcAyu9cFGwhl3KYyaqa
# JGsHG0F1lTidxid7
