...
try
{
    # For example: $Name = 'Connection Name'
    $Name = ''
    
    # For example: $NewName = 'Connection New Name'
    $NewName = ''

    # For example: $Agents = @('Agent Name1','Agent Name2')
    $Agents = @()

    # For example: $OnlineConnection = New-SharePointConnectionOnlineObject -Username  'Service account'  -Password 'Password' -AppProfileName 'App Profile Name'   -CacServiceAccountName 'CAC Service Account Name' -DelegateAppName 'Delegate Name' -CustomAdminUrl 'SharePoint Admin Center URL' -ServiceAccountAuthentication Legacy\Modern\CAC
    # If you want to keep the original option, delete the corresponding parameters from the New-SharePointConnectionOnlineObject cmdlet. 
    # For example:
    # To keep the original 'CustomAdminUrl', delete the '-CustomAdminUrl ''' parameter from the New-SharePointConnectionOnlineObject cmdlet.
    $OnlineConnection = New-SharePointConnectionOnlineObject -Username  ''  -Password '' -AppProfileName ''  -CacServiceAccountName '' -DelegateAppName '' -CustomAdminUrl '' -ServiceAccountAuthentication Legacy
    
    # example of On-Premises connection:
    # $OnPremisesConnection = New-SharePointConnectionOnPremisesObject -Username 'Service account'  -Password 'Password' -CentralAdministrationURL 'SharePoint central administration'
    # $ConnectionCollection = New-UpdateSharePointConnectionCollectionObject -OnPremisesConnectionOption $OnPremisesConnection -Agents $Agents -Name $Name -NewName $NewName

    # If you only want to update the connection name, please remove the following '-OnlineConnectionOption $OnlineConnection' parameter.
    $ConnectionCollection = New-UpdateSharePointConnectionCollectionObject -OnlineConnectionOption $OnlineConnection -Agents $Agents -Name $Name -NewName $NewName

	$Response = Update-SPConnection -APIKey $ApiKey -BaseUri $BaseUri -Model $ConnectionCollection
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCruQi8rk55GsmR
# tOp9Gl4kiQI/x8Qv3PubYRIBUNH5VaCCDZowggawMIIEmKADAgECAhAIrUCyYNKc
...
# SIb3DQEJBDEiBCCbbJga0mjfyO2GwGsv5djZCx1IiIOv7XMrM35m11NYdzANBgkq
# hkiG9w0BAQEFAASCAYAhrQE1RpcKwnsWvAr7DiUqbqXdBlkCY+N7ijQ6zId1P2PY
# pDZHoQYdNMDMqqAVcT5l/FsSCWHEz+2SdkqXsZEU340j2xeYJ6qTrGhEY8Ido8f6
# 3BewjhmCJx1AUg/wOAl0NWyzHFviIhgKEhaltE3UvJTAYm2KjnBTF0Hg9hJCy475
# gN+/3/uJ2CTgX9Pf9NfM0t1CPbanmkUGIgM3LUB74/sLqVlyj0OIIIT7Oqahk9lu
# irE1tqcGIDUJH38tXnlD8tQiBNBZC2rAQ1m6yVjuFD2/lCd6xSHa0RC9hi9gNPaD
