...
{
    class AddIMAPPOP3Plan : AbstractApplication
    {
        static void Main(string[] args)
        {
            new AddIMAPPOP3Plan().RunAsync().Wait();
        }
        ...
        /// <summary>
        /// <see cref="GetIMAPPOP3MigrationPolicy"/>
        /// </summary>
        ...
        {
            var mappingContent = new IMAPPOP3MappingContentModel
            {
                Source = new IMAPPOP3MailBoxObject
                {
                    Mailbox = "<mailbox>",
                    Password = "<password>",
                },
                DestinationMailbox = "<exchange mailbox>",
                MigrateArchivedMailboxOrFolder = true,
            };

            var mappings = new IMAPPOP3MappingModel
            {
                Source = new IMAPPOP3ConnectionModel
                {
                    Type = "Outlook",
                ...
                Contents = new List<IMAPPOP3MappingContentModel>
                {
                    mappingContent
                },
            };

            var planSettings = new IMAPPOP3PlanSettingsModel
            {
                NameLabel = new PlanNameLabel
                {
                    Name = $"CSharp_IMAP-POP3_Plan_{DateTime.Now.ToString("yyyyMMddHHmmss")}",
            ...
            var plan = new IMAPPOP3PlanModel
            {
                Mappings = mappings,
            ...
            var response = await client.PostAsync("/api/imap-pop3/plans", content);
